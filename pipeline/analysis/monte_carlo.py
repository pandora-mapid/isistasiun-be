...
Uncertainty is *propagated, not estimated*: each variable carries an empirical
distribution built from repeated observation blocks, and the four are drawn
...
# One (F, E, C, V) tuple of distributions per category.
...
def _sum_across_categories(dists: CategoryDists, n_iterations: int) -> np.ndarray:
    ...
    for f, e, c, v in dists.values():
        total += simulate_category_spending(f, e, c, v, n_iterations)
    return total
...
def load_distributions_from_db(station_id: str, time_slot: str) -> tuple[CategoryDists, CategoryDists]:
    ...
    c_by_cat: dict[str, list[float]] = {c: [] for c in BAKU_CATEGORIES}
    gerai_flow_by_cat: dict[str, list[float]] = {c: [] for c in BAKU_CATEGORIES}
    for category, passers, entered, completed in ec_rows:
        ...
        c_by_cat[category].append(completed / entered if entered else 0.0)
    ...
    for category in BAKU_CATEGORIES:
        if not (e_by_cat[category] and c_by_cat[category] and v_by_cat[category]):
            continue  # category lacks the samples to simulate — contributes 0
        e = VariableDistribution.from_values(e_by_cat[category])
        c = VariableDistribution.from_values(c_by_cat[category])
        v = VariableDistribution.from_values(v_by_cat[category])
        potential[category] = (entrance_flow, e, c, v)
        if gerai_flow_by_cat[category]:
            captured[category] = (
                VariableDistribution.from_values(gerai_flow_by_cat[category]), e, c, v,
            )
